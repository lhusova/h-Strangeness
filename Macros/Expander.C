...
THnF* GetTHnF(TFile *file, TString newHistoName = "newHistogram", TString histoName = "correlate-strangeness/sameEvent/Signal/K0Short"){
  ...
  nbins[3] = hVtxZ->GetNbinsX();
  nbins[4] = hMult->GetNbinsX();
  ...
  THnF *hReturnHisto = new THnF(newHistoName.Data(), "", 5, nbins, expandedAxes);
      ...
      for(Int_t i3=0; i3<nbins[2]; i3++){
        for(Int_t i4=0; i4<nbins[3]; i4++){
          for(Int_t i5=0; i5<nbins[4]; i5++){
            ...
            coordinateNew[0] = i1+1; coordinateNew[1] = i2+1; coordinateNew[2] = i3+1; coordinateNew[3] = i4+1; coordinateNew[4] = i5+1;
