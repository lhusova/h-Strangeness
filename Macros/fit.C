...
#include <TString.h>
      ...
      Double_t BackgroundFunc1(const Double_t* x, const Double_t* par)
    {
    Double_t xVal = x[0];
    if (xVal > 0.45 && xVal < 0.535) {
        TF1::RejectPoint();
        return 0;
    }
  ...
  //  Double_t background = par[0];
    ...
    Double_t BackgroundFunc2(const Double_t* x, const Double_t* par)
    {
    Double_t xVal = x[0];
    if (xVal > 1.101 && xVal < 1.135) {
        TF1::RejectPoint();
        return 0;
    }
    Double_t background = par[0] + par[1] * xVal + par[2]*xVal*xVal;
  //  Double_t background = par[0];
   ...
   /* Double_t BackgroundFunc3(const Double_t* x, const Double_t* par)
    {
    Double_t xVal = x[0];
    if (xVal > 1.30 && xVal < 1.34) {
        TF1::RejectPoint();
        return 0;
    }
    Double_t background = par[0] + par[1] * xVal + par[3]*xVal*xVal;
    return background;
    }

    Double_t BackgroundFunc4(const Double_t* x, const Double_t* par)
    {
    Double_t xVal = x[0];
    if (xVal > 1.651 && xVal < 1.69) {
        TF1::RejectPoint();
        return 0;
    }
    Double_t background = par[0] + par[1] * xVal + par[3]*xVal*xVal;
...
void fit(){
    ...
    TF1 *fullfit = new TF1( "fullfit", "[0]+[1]*x+[2]*x*x+gaus(3)", 0.45, 0.535);
    TF1 *bgright = new TF1( "bgright", "[0]+[1]*x", 0.4, 0.6);
    //TF1 *bgleft = new TF1( "bgleft", BackgroundFunc1, 0.4, 0.47, 3);
  //  TF1 *signal = new TF1( "signal", "gaus", 0.4, 0.6);
    ...
    for (int i=0; i<7; i++){
        ...
        TCanvas *c = new TCanvas();
              TF1 *bgleft = new TF1( "bgleft", BackgroundFunc1, 0.4, 0.6, 3);
              bgleft->SetParameter(0, -1.65e7);
              bgleft->SetParameter(1, 8.93e7);
              bgleft->SetParameter(2, -1.02e8);
  
        
        //if (i==1|i==2) TF1 *bgleft = new TF1( "bgleft", BackgroundFunc2, 1.07, 5, 3);
        //if (i==3|i==4) TF1 *bgleft = new TF1( "bgleft", BackgroundFunc3, 0, 5, 3);
       // if (i==5|i==6) TF1 *bgleft = new TF1( "bgleft", BackgroundFunc4, 0, 5s, 3);
        for (int j=0; j<10; j++){
    ...
    TF1 *meanfitK0 = new TF1( "meanfitK0", "[0]+[1]*x", 2, 10);
