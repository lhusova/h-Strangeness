...
void CompareBackgroundDPhi(Int_t part=0,Int_t multClass =1,Int_t pt=2, Int_t ptTriggBin =1){
  ...
  Double_t ptBins[]={0.,1.,2.,3.,3.5,4.,5.,6.,7.,8.,10.,12,15};
  ...
  Color_t colors[n] = {kGreen+2, kBlue+1,kMagenta};//,kGreen+2};

  TH1F * phiProj_bck[n];

  for (Int_t i = 0; i < n; i++) {
    fFile[i] = new TFile(Form("../data/LongTrain_2Aug/%s/Yields_%s_%s%s_ptTrigg%i.root",nameSave[part].Data(),nameSave[part].Data(),multiplicityNames[multClass].Data(),file_Suffix[i].Data(),ptTriggBin));
  ...
  Plotter::SetHistAxes(phiProj,"#Delta#varphi (rad)","1/#font[52]{N}_{Trigg}d#font[52]{N}/d#Delta#varphi (rad^{-1})");
  Plotter::SetHist(phiProj,"",20,kBlack,0.8);
  phiProj->GetYaxis()->SetMaxDigits(3);
  phiProj->DrawCopy();
  ...
  pave->AddText(Form("%g < #font[52]{p}^{assoc}_{T} < %g GeV/#font[52]{c}",ptBins[pt],ptBins[pt+1]));
