...
void fitcascade(){
    ...
    TF1 *fullfit = new TF1( "fullfit", "[0]+[1]*x+gaus(2)", 0, 2);
    
    //create a .root file to save the outputhists.
    TFile *file_Tuple = new TFile ("tuple.root","recreate");
    for (int i=0; i<7; i++){
        fullfit->SetParameters(parameter0[i],parameter1[i],parameter2[i],parameter3[i],parameter4[i]);
        ...
        for (int j=0; j<10; j++){
            ...
            peakPosition[i][j]=fullfit->GetParameter(3);
            uncertaintyMean[i][j]=fullfit->GetParError(3);
            peakWidth[i][j]= abs(fullfit->GetParameter(4));
            uncertaintyWidth[i][j]=fullfit->GetParError(4);
