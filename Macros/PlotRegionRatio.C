...
void PlotRegionRatio(Int_t iPart = 0, Int_t RegionNum = 0, Int_t RegionDenom = 1)
{
  ...
  for (Int_t iPtTrigg = 0; iPtTrigg < nPtTriggBins; iPtTrigg++)
  {
    ...
    for (Int_t iMult = 0; iMult < nMultBins; iMult++)
    {
      ...
      fHistSpectrumSistMultRatio[iMult] = (TH1F *)fProjSyst[iMult][iPtTrigg][RegionNum]->Clone(Form("fHistSpectrumSistRatio_%s_pttrigg%i_%s", multiplicityNames[iMult].Data(), iPtTrigg, namesRegions[RegionNum].Data()));

      fHistSpectrumStatMultRatio[iMult]->Divide(fProj[ChosenMult][iPtTrigg][RegionDenom]);
      fHistSpectrumSistMultRatio[iMult]->Divide(fProjSyst[ChosenMult][iPtTrigg][RegionDenom]);

      ErrRatioCorr(fProj[iMult][iPtTrigg][RegionNum], fProj[ChosenMult][iPtTrigg][RegionDenom], fHistSpectrumStatMultRatio[iMult], 0);
      ErrRatioCorr(fProjSyst[iMult][iPtTrigg][RegionNum], fProjSyst[ChosenMult][iPtTrigg][RegionDenom], fHistSpectrumSistMultRatio[iMult], 0);
