...
void PlotRegionRatio(Int_t iPart = 0, Int_t RegionNum=0, Int_t RegionDenom = 1)
{
  ...
  for (Int_t iPtTrigg = 0; iPtTrigg < 3; iPtTrigg++)
  {
    ...
    for (Int_t iReg = 0; iReg < 3; iReg++)
    {
      ...
      for (Int_t iMult = 0; iMult < 9; iMult++)
      {
        ...
        if (iMult != 0)
          legendAllMult->AddEntry(fProjScaled[iMult][iPtTrigg][iReg], Form("%s %s", paveRegionsShort[iReg].Data(), multiplicityPave[iMult].Data()) + sScaleFactorFinal[iMult] + " ", "pef");
      } // end loop on mult
    }
    LegendTitle->Draw("");
    legendAllMult->Draw("");
    ...
    TString TitleYSpectraRatio = Form("%s / %s", namesRegions[RegionNum].Data(), namesRegions[RegionDenom].Data());
    ...
    for (Int_t iMult = 0; iMult < 9; iMult++)
    {
      if (iMult != 0 && iMult != 1 && iMult != 8)
        continue; // keep only 0-1%, 70-100% and 0-100%
      iMultEff++;
      fHistSpectrumStatMultRatio[iMult] = (TH1F *)fProj[iMult][iPtTrigg][RegionNum]->Clone(Form("fHistSpectrumStatRatio_%s_pttrigg%i_%s", multiplicityNames[iMult].Data(), iPtTrigg, namesRegions[RegionNum].Data()));
      fHistSpectrumSistMultRatio[iMult] = (TH1F *)fProj[iMult][iPtTrigg][RegionNum]->Clone(Form("fHistSpectrumSistRatio_%s_pttrigg%i_%s", multiplicityNames[iMult].Data(), iPtTrigg, namesRegions[RegionNum].Data()));

      fHistSpectrumStatMultRatio[iMult]->Divide(fProj[ChosenMult][iPtTrigg][RegionDenom]);
      fHistSpectrumSistMultRatio[iMult]->Divide(fProj[ChosenMult][iPtTrigg][RegionDenom]);

      ErrRatioCorr(fProj[iMult][iPtTrigg][RegionNum], fProj[ChosenMult][iPtTrigg][RegionDenom], fHistSpectrumStatMultRatio[iMult], 0);
      ErrRatioCorr(fProj[iMult][iPtTrigg][RegionNum], fProj[ChosenMult][iPtTrigg][RegionDenom], fHistSpectrumSistMultRatio[iMult], 0);
      ...
      fHistSpectrumStatMultRatio[iMult]->SetMarkerColor(colRegions[RegionNum][iMultEff]);
      fHistSpectrumStatMultRatio[iMult]->SetLineColor(colRegions[RegionNum][iMultEff]);
      ...
      fHistSpectrumSistMultRatio[iMult]->SetMarkerColor(colRegions[RegionNum][iMultEff]);
      fHistSpectrumSistMultRatio[iMult]->SetLineColor(colRegions[RegionNum][iMultEff]);
    ...
    TString stringoutpdf = Form("../../Ptspectrum_%s_%svs%s_ptTrigg%d", particleName[iPart].Data(), namesRegions[RegionNum].Data(), namesRegions[RegionDenom].Data(), iPtTrigg);
